...
function Nomina() {
  ...
  const cargarDatos = async () => {
    ...
    try {
      ...
      let activos = 0;
      try {
        const resEmp = await fetchConAuth('${API_URL}/api/usuarios');
      ...
      if (esGerente || esContador) {
        try {
          const resDesc = await fetchConAuth('${API_URL}/api/descuentos?estado=activo');
      ...
      let nominas = [];
      try {
        const resNom = await fetchConAuth('${API_URL}/api/nomina?anio=2026');
      ...
      if (esGerente) {
        try {
          const resExt = await fetchConAuth('${API_URL}/api/asistencia/proyecto/all');
