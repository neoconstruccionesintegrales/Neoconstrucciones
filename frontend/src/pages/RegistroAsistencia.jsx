...
function RegistroAsistencia() {
  ...
  const cargarProyectos = async () => {
    try {
      const res = await fetchConAuth(`${process.env.REACT_APP_API_URL || 'http://localhost:5000'}/api/proyectos`);
  ...
  const cargarRegistrosDia = async () => {
    ...
    try {
      const url = proyectoSeleccionado
        ? `${process.env.REACT_APP_API_URL || 'http://localhost:5000'}/api/asistencia/proyecto/${proyectoSeleccionado}?fecha=${fecha}`
        : `${process.env.REACT_APP_API_URL || 'http://localhost:5000'}/api/asistencia?fecha=${fecha}&email=${form.email}`;
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await fetchConAuth(`${process.env.REACT_APP_API_URL || 'http://localhost:5000'}/api/asistencia`, {
