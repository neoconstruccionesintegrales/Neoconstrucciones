...
function GestionDescuentos() {
  ...
  const cargarDescuentos = async () => {
    ...
    try {
      ...
      const res = await fetchConAuth(`${API_URL}/api/descuentos?${query}`);
  ...
  const guardarDescuento = async (e) => {
    e.preventDefault();
    try {
      const res = await fetchConAuth('${API_URL}/api/descuentos', {
        method: 'POST',
        body: JSON.stringify({
          ...form,
  ...
  const cambiarEstado = async (id, nuevoEstado) => {
    try {
      const res = await fetchConAuth(`${API_URL}/api/descuentos/${id}/estado`, {
        method: 'PATCH',
  ...
  const cancelarDescuento = async (id) => {
    ...
    try {
      const res = await fetchConAuth(`${API_URL}/api/descuentos/${id}/cancelar`, { method: 'PATCH' });
      const data = await res.json();
      if (data.success) {
