  ...
  const cargarDatos = async () => {
    ...
    try {
      const [resProyectos, resCotizaciones, resClientes] = await Promise.all([
        ...
        axios.get(`${API_URL}/proyectos/cotizaciones-aprobadas`).catch(() => ({ data: { data: [] } })),
...
${facturasPagadas.join('\n')}`);
  ...
  const verDetalle = async (proyecto) => {
    try {
      const res = await axios.get(`${API_URL}/proyectos/${proyecto.idProyecto}`);
  ...
  const guardarSeguimiento = async () => {
    try {
      await axios.post(`${API_URL}/proyectos/${proyectoDetalle.idProyecto}/seguimientos`, seguimientoData, { 
                                  ...
                                  {hito.facturaGenerada && facturaAsociada?.estado === 'Pagada' && !hito.completado && !hito.cubiertoPorSaldo && (
                                    <button
                                      onClick={async () => {
                                        ...
                                        try {
                                          await axios.post(
                                            `${API_URL}/proyectos/${proyectoDetalle.idProyecto}/hitos/${hito.idHito}/completar`,
                                            {},
                                            { headers: { Authorization: `Bearer ${localStorage.getItem('token')}` } }
                                          );
