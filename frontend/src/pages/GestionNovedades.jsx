...
function GestionNovedades() {
  ...
  const cargarNovedades = async () => {
    try {
      const res = await fetchConAuth(`${process.env.REACT_APP_API_URL || 'http://localhost:5000'}/api/novedades`);
      const data = await res.json();
  ...
  const aprobarNovedad = async (id) => {
    try {
const res = await fetchConAuth(`${process.env.REACT_APP_API_URL || 'http://localhost:5000'}/api/novedades/${id}/aprobar`, { method: 'PUT' });      const data = await res.json();
      if (data.success) {
