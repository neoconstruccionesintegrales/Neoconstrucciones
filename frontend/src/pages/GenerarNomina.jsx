...
function GenerarNomina() {
  ...
  const cargarNominasExistentes = async () => {
    try {
      const res = await fetchConAuth(`http://localhost:5000/api/nomina?anio=${form.anio}`);
      const data = await res.json();
  ...
  const calcular = async (e) => {
    ...
    setMensaje('');
    try {
      const res = await fetchConAuth('http://localhost:5000/api/nomina/calcular', {
        method: 'POST',
  ...
  const aprobar = async () => {
    ...
    try {
      const res = await fetchConAuth(`http://localhost:5000/api/nomina/${resultado.idNomina}/aprobar`, { method: 'PUT' });
      const data = await res.json();
      if (data.success) {
  ...
  const marcarPagada = async (idNomina) => {
    ...
    try {
      const res = await fetchConAuth(`http://localhost:5000/api/nomina/${idNomina}/pagar`, { method: 'PUT' });
      const data = await res.json();
      if (data.success) {
