...
function GenerarNomina() {
  ...
  const cargarNominasExistentes = async () => {
    try {
      const res = await fetchConAuth(`${API_URL}/api/nomina?anio=${form.anio}`);
      const data = await res.json();
  ...
  const calcular = async (e) => {
    ...
    setMensaje('');
    try {
      const res = await fetchConAuth('${API_URL}/api/nomina/calcular', {
        method: 'POST',
  ...
  const aprobar = async () => {
    ...
    try {
      const res = await fetchConAuth(`${API_URL}/api/nomina/${resultado.idNomina}/aprobar`, { method: 'PUT' });
      const data = await res.json();
      if (data.success) {
  ...
  const marcarPagada = async (idNomina) => {
    ...
    try {
      const res = await fetchConAuth(`${API_URL}/api/nomina/${idNomina}/pagar`, { method: 'PUT' });
      const data = await res.json();
      if (data.success) {
