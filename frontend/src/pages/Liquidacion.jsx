...
// Detectar rol del usuario desde localStorage o default
  ...
  const verificarInasistencias = async (emailSel, fechaSel) => {
    ...
    try {
      const res = await fetchConAuth(`http://localhost:5000/api/nomina/verificar-inasistencias?email=${emailSel}&fechaFinal=${fechaSel}`);
      const data = await res.json();
      if (data.success) {
  ...
  const verificarVacaciones = async (emailSel, fechaSel) => {
    if (!emailSel || !fechaSel) return;
    try {
      const res = await fetchConAuth(`http://localhost:5000/api/nomina/verificar-vacaciones?email=${emailSel}&fechaFinal=${fechaSel}`);
      const data = await res.json();
      if (data.success) {
  ...
  const liquidar = async (e) => {
    ...
    try {
      ...
      const res = await fetchConAuth('http://localhost:5000/api/nomina/liquidar', {
        method: 'POST',
