...
function Servicios() {
    ...
    const obtenerServicios = useCallback(async () => {
        try {
            const res = await fetchConAuth(`${process.env.REACT_APP_API_URL || 'http://localhost:5000'}/api/servicios`);
            if (res.status === 401) {
    ...
    const enviarServicio = async (e) => {
        ...
        const url = modoEdicion
            ? `${process.env.REACT_APP_API_URL || 'http://localhost:5000'}/api/servicios/${nuevoServicio.idServicio}`
            : `${process.env.REACT_APP_API_URL || 'http://localhost:5000'}/api/servicios`;
        try {
